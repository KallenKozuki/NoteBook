...
**==DFS和回溯的区别：==**DFS是一个劲的往某个方向搜索，而回溯算法是建立在DFS的基础之上的，不同的是在搜索的过程中，达到结束条件后，回复状态，回溯上一层，再次搜索。因为二者的区别就是==有无重置状态==

**==何时使用回溯算法：==**

### 78 子集#

**==解法1：迭代（枚举）==**		***T=O(n2^n^), S=O(n)***
